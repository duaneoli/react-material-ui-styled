...
class SessionStorage {
  ...
  static set = <T>(key: string, value: T) =>
    sessionStorage.setItem(KEY_PREFIX_SESSION.concat(key), JSON.stringify(value))

  static remove = (key: string) => sessionStorage.removeItem(KEY_PREFIX_SESSION.concat(key))
  ...
  static getAll = (): { [key: string]: any } => {
    ...
    for (let x = 0; x < sessionStorage.length; x++) {
      ...
      if (key)
        sessionStorageObject[key.replace(KEY_PREFIX_SESSION, '')] = JSON.parse(localStorage.getItem(key) as string)
    }
